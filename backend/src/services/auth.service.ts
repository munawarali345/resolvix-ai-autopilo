...
if (!isMatch) {
  ...
  //  successfully login hu ne k bad log hu jaiga 
   await createAuditLog({ userId: user._id.toString(), action: AuditAction.LOGIN, });
...
export const verifyEmailService = async (
  ...
  const user = await User.findOne({ // Kis user ka verificationToken = abc123 hai?
    verificationToken: token,
  });

  if (!user) { // Agar token DB me mila hi nahi to 
